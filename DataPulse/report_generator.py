...
class ReportGenerator:
    ...
    def generate_sales_report(self, session_data):
        ...
        try:
            ...
            html_content = self._render_sales_html(stats, processed_data, plot_base64)
    ...
    def generate_forecast_report(self, session_data):
        ...
        try:
            ...
            model_info = self._get_model_info(model_accuracy)
            
            # Создаем график
            plot_base64 = self._create_forecast_plot(limited_historical, forecast_results, model_info)
            
            # Генерируем HTML
            html_content = self._render_forecast_html(forecast_results, model_info, plot_base64)
            
            return self._html_to_pdf(html_content)
            
        except Exception as e:
            self.logger.error(f"Ошибка генерации отчета по прогнозам: {e}")
            return None

    def generate_full_report(self, session_data):
        ...
        try:
            ...
            full_historical_stats = self._calculate_sales_statistics(pd.DataFrame(processed_data))
            forecast_stats = self._calculate_forecast_statistics(forecast_results)
            model_info = self._get_model_info(model_accuracy)
            
            # График
            plot_base64 = self._create_full_plot(limited_historical, forecast_results, 
                                              full_historical_stats, forecast_stats, model_info)
            
            # HTML - используем ограниченные исторические данные для отображения
            html_content = self._render_full_html(limited_historical, forecast_results, 
    ...
    def _calculate_sales_statistics(self, df):
        ...
        avg_growth = self._calculate_average_growth(sales)
        ...
        if not sales.empty:
            ...
            if 'date' in df.columns:
                ...
                best_day_info = self._format_date_for_display(best_day_date)
                worst_day_info = self._format_date_for_display(worst_day_date)
        else:
            best_day_sales = 0
            worst_day_sales = 0
        ...
        return {
            ...
            'growth_rate': self._calculate_growth_rate(sales),
    ...
    def _calculate_average_growth(self, sales_data):
    ...
    def _get_limited_historical_data(self, processed_data, days=60):
    ...
    def _get_model_info(self, model_accuracy):
    ...
    def _create_sales_plot(self, df, stats):
        ...
        try:
            ...
            self.logger.error(f"Ошибка создания графика продаж: {e}")
            return None

    def _create_forecast_plot(self, historical_data, forecast_results, model_info):
        ...
        try:
            ...
            self.logger.error(f"Ошибка создания графика прогноза: {e}")
            return None

    def _create_full_plot(self, historical_data, forecast_results, hist_stats, fc_stats, model_info):
        ...
        try:
            ...
            self.logger.error(f"Ошибка создания полного графика: {e}")
            return None
    ...
    def _plot_to_base64(self):
        ...
        try:
            ...
            self.logger.error(f"Ошибка конвертации графика: {e}")
            return None
    ...
    def _format_date_for_display(self, date_obj):
        ...
        try:
            ...
            self.logger.warning(f"Ошибка форматирования даты {date_obj}: {e}")
            return str(date_obj)

    def _get_day_name_from_date(self, date_obj):
        ...
        try:
            ...
            self.logger.warning(f"Ошибка преобразования даты {date_obj}: {e}")
            return 'Неизвестно'

    def _get_day_name_from_number(self, day_num):
    ...
    def _render_forecast_html(self, forecast_results, model_info, plot_base64):
        ...
        return template.render(
            ...
            format_date=self._format_date_for_display,
            get_day_name=self._get_day_name_from_date,
    ...
    def _render_sales_html(self, stats, historical_data, plot_base64):
        ...
        return template.render(
            ...
            plot_base64=plot_base64,
            format_date=self._format_date_for_display,
            get_day_name_historical=self._get_day_name_from_number,
            get_month_name=self._get_month_name
        )

    def _render_full_html(self, historical_data, forecast_results, hist_stats, fc_stats, model_info, plot_base64):
