...
def generate_full_report(session_data):
    ...
    try:
        ...
        # Функции для получения названий дней и месяцев
        def get_day_name(date_str):
            """Для прогнозов: преобразует строку даты в название дня недели"""
            try:
                if hasattr(date_str, 'strftime'):  # Если это объект datetime/Timestamp
                    date_obj = date_str
        ...
        def get_day_name_historical(day_number):
            """Для исторических данных: преобразует номер дня недели в название"""
            try:
                days = ['Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота', 'Воскресенье']
                if isinstance(day_number, (int, float)) and 0 <= int(day_number) < 7:
                    return days[int(day_number)]
                else:
                    return 'Неизвестно'
            except Exception as e:
                print(f"Ошибка при преобразовании номера дня '{day_number}': {e}")
                return 'Неизвестно'
        ...
        def get_month_name(month_number):
            ...
            try:
                months = ['Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь',
                         'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь']
                if isinstance(month_number, (int, float)) and 1 <= int(month_number) <= 12:
                    return months[int(month_number) - 1]
                else:
                    return 'Неизвестно'
            except Exception as e:
                print(f"Ошибка при преобразовании номера месяца '{month_number}': {e}")
                return 'Неизвестно'
        ...
        html_content = template.render(
            ...
            plot_base64=plot_base64,
            get_day_name=get_day_name,
            get_day_name_historical=get_day_name_historical,
            get_month_name=get_month_name,
            model_name=model_name,
            accuracy_value=accuracy_value,
            accuracy_date=accuracy_info.get('created_at', 'Неизвестно')[:10] if accuracy_info else 'Неизвестно',
